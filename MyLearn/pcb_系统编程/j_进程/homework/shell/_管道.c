...
int main(){
	...
	char* strout[10];
	for(int i = 0; i < 10; i++){
		strout[i] = (char*)malloc(20*sizeof(char));//直接sizeof(char)也可以，原因未探究
	}
#if 0
	...
	while(1){
		...
		switch (i){
			...
			//	memset(strout[j], 0,20*sizeof(char));  设了NULL就别用memset了，用下面的释放了重新申请 
					free(strout[j]);
					strout[j]=(char*)malloc(20*sizeof(char));
				}
